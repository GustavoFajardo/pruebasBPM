...
export const TSuscription: React.FC<ISuscription> = () => {
  ...
  const getListCharacterization = (idCustomer: number) => {
    ...
    let aux: any = [];
    let auxSorted: any = [];
    _configService.getCharacterizationByCustomerType(idCustomer).subscribe((resp: any) => {
      if (resp) {
  ...
  const closeForm = (status: boolean) => {
    ...
    listAllDocuments(beanProcedureImp.DataBeanProperties.IDProcedureImp,
  ...
  const onChangeSelect_Process = (e: any) => {
    return e == null ? null : (setValue("Tramite", e.id), clearErrors('Tramite'), getOfficeCatalogForBusinessProcess(parseInt(e.id)));
  };
              ...
              <div className="row mt-5">
                ...
                <Col sm={6} className="mt-3">
                  <TextField
                    ...
                    onChange={(e) => { setValue("CustomerType", e.target.value); getListCharacterization(parseInt(e.target.value)) }}
                  >
                ...
                <Col sm={6} className="mt-3">
                  <Autocomplete
                    ...
                    onChange={(e, value: any) => { setValue("Caracterizacion", value ? value.id : 0); clearErrors('Caracterizacion'); getProcess(value !== null ? value.id : 0) }}
                    renderInput={(params) => <TextField {...params} label=".:Selecione una caracterización:." fullWidth />}
                  />
                  <span className="text-danger">
                    {errors
                ...
                <Col sm={6} className="mt-3">
                  <Autocomplete
                    ...
                    renderInput={(params) => <TextField {...params} label=".:Trámite:." fullWidth />}
                  />
                  <span className="text-danger">
                    {errors.entity
                ...
                <Col sm={6} className="mt-3">
                  {listOffice.length > 0
                    ? <Autocomplete
                      ...
                      onChange={(e, value: any) => { setValue("Sucursal", value ? value.id : 0); clearErrors('Sucursal'); setTramite(value ? value.label : "No selected") }}
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          size="small"
                    ...
                    : <TextField
                      size="small"
                      fullWidth
                      value={tramite === "" ? "" : "No hay seccionales asignadas a este trámite"}
                      label=".:Seleccione una Seccional:."
                      id="state"
                    >
                    </TextField>
                ...
                <Col sm={12} className="d-flex justify-content-center mt-3 mb-5">
                  <ThemeProvider theme={inputsTheme}>
                    <Button className="w-5" type="submit" color="secondary" variant="contained">
                      CONTINUAR
                    </Button>
                  </ThemeProvider>
                </Col>
          ...
          <div className="d-flex mt-2 flex-column w-100 ">
            ...
            {showModalForm && (
              <SLoadForm
                ...
                title={titleDoc}
                IDProcedureIMP={beanProcedureImp.DataBeanProperties.IDProcedureImp}
              />
            )}
          </div>
        );
