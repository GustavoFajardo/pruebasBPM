...
export const TTrayForVerify: React.FC<ITrayForVerify> = () => {
    ...
    const getProcedureImpForVerify = async () => {
        setShowSpinner(true);
        await _suscripcionService.getProcedureImpForVerify(parseInt(getSesion().IDAccount)).then((resp: any) => {
            console.log(resp);
    ...
    const getProcedureActionForVerify = async (IDProcedureImp: number) => {
        await _suscripcionService.getProcedureActionForVerify(parseInt(getSesion().IDAccount), IDProcedureImp).then((resp: any) => {
            if (resp.data.DataBeanProperties.ObjectValue) {
    ...
    const aprobarDoc = async () => {
        await _suscripcionService.verifyProcedureAction(IDProcedureAction, "aprobado!!").then((resp: any) => {
            if (resp.data.DataBeanProperties.ObjectValue) {
    ...
    const rechazar = async (descripcion: string) => {
        ...
        await _suscripcionService.declineProcedureAction(IDProcedureAction, descripcion).then((resp: any) => {
            if (resp.data.DataBeanProperties.ObjectValue) {
                getProcedureActionForVerify(beanProcedureImp.IDProcedureImp);
                closeModal();
                if (listDocuments.length === 0) {
    ...
    const renderSwitch = (param: number) => {
        switch (param) {
            case 0:
                return <div className="w-100 d-flex h-100 flex-wrap justify-content-xl-start justify-content-sm-center">
                    {listProcedureImp.length > 0 ? (
                                ...
                                <Table stickyHeader aria-label="sticky table" className={classes.root}>
                                    ...
                                    <TableBody>
                                        {listProcedureImp.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item: any) => (
                                            <TableRow hover key={item.DataBeanProperties.IDProcedureImp} role="checkbox" tabIndex={-1}>
                                                            ...
                                                            <ThemeProvider theme={inputsTheme}>
                                                                <Tooltip title="Seleccionar">
                                                                    <Button
                                                                        ...
                                                                        color="secondary"
                                                                        onClick={() => seleccionar(item.DataBeanProperties)}>
                                                                        <BsCheckSquare />
                                                                    </Button>
                                                                </Tooltip>
                                                            </ThemeProvider>
                        ...
                        : (
                            <div className="w-100 d-flex justify-content-center">
                                <h1 className="text-muted h-100">No hay procesos por Verificar :(</h1>
                            </div>
                        )
                    }
                </div>
            case 1: return <div>
                <div className="w-100 d-flex flex-wrap justify-content-lg-start justify-content-sm-center">
                            ...
                            <Table stickyHeader aria-label="sticky table" className={classes.root}>
                                ...
                                <TableBody>
                                    {listDocuments.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((item: any) => (
                                        <TableRow key={item.DataBeanProperties.IDAction} hover role="checkbox" tabIndex={-1}>
                                            <TableCell>{item.DataBeanProperties.ProcedureActionName}</TableCell>
                                            <TableCell>{item.DataBeanProperties.UptoDate}</TableCell>
                                            <TableCell>{item.DataBeanProperties.AccountID}</TableCell>
                                            <TableCell>{item.DataBeanProperties.AccountName}</TableCell>
                                            {item.DataBeanProperties.Url && <TableCell> <a href={item.DataBeanProperties.Url}>
                                                {item.DataBeanProperties.Media}
                                            </a></TableCell>}
                                            {(item.DataBeanProperties.IDForm && item.DataBeanProperties.ResponseJsonValue) && <TableCell className="textDecoration" onClick={() => (statusModalForm(true), setBeanAction(item.DataBeanProperties), setTitleDoc(item.DataBeanProperties.Name))}>
                                                {item.DataBeanProperties.ProcedureName}
                                            </TableCell>}
                                            <TableCell>
                                                <div className="d-lg-flex d-none">
                                                    <Tooltip title="Aprobar Documento">
                                                        <Button className="mr-2 mt-2 mb-2" variant="contained" color="success" onClick={() => (setShowDelete(true), setProcedureAction(item.DataBeanProperties.IDAction))}>
                                                            <BsCheck2 />
                                                        </Button>
                                                    </Tooltip>
                                                    <Tooltip title="Rechazar Documento">
                                                        <Button className="ml-2 mt-2 mb-2" variant="contained" color="error" onClick={() => openModal(item.DataBeanProperties.IDAction)}>
                                                            <BsXSquare />
                                                        </Button>
                                                    </Tooltip>
                                                </div>
                                                <div className="d-block d-lg-none">
                                                    ...
                                                    >
                                                        <SpeedDialAction
                                                            ...
                                                            onClick={() => (setShowDelete(true), setProcedureAction(item.DataBeanProperties.IDAction))}
                                                        // aprobarDoc(item.DataBeanProperties.IDAction)
                                                        />
                                                        <SpeedDialAction
                                                            onClick={() => openModal(item.DataBeanProperties.IDAction)}
                                                            icon={<BsXSquare />}
                    ...
                    {showModalForm && <SLoadForm beanAction={beanAction} status={showModalForm} title={titleDoc} type={2} getShowForm={statusModalForm} IDProcedureIMP={beanProcedureImp.IDProcedureImp} />}
                    {show &&
                        <Modal size="sm" show={show} backdrop="static" centered keyboard={false}>
                            <Modal.Header>
                                Rechazar
                                ...
                                <Modal.Body>
                                    <Row className="mt-3">
                                        <Col sm={12}>
                                            ...
                                            />
                                            <span className="text-danger">{errors ? errors.Description?.type === 'required' && "El campo Descripción es obligatorio." : ''}</span>
                                        </Col>
                                    </Row>
                                </Modal.Body>
                                <Modal.Footer>
            ...
            default:
                return <div>
                    ...
                    <h5 className="w-100 text-center">Muchas Gracias!, Ya quedo gestionado el proceso. Para seguir con otro proceso click <b onClick={() => { handleReset(); getProcedureImpForVerify() }}> <u> AQUI</u></b></h5>
                </div>;
            ...
            <div className="nWhite p-3 m-3 w-100">
                <Box sx={{ width: '100%' }}>
                    ...
                    <Stepper activeStep={activeStep} className={classes.root}>
                        {steps.map((label, index) => (
                            <Step key={index} completed={completed[index]} disabled={completed[index]}>
                ...
                <div>
                    {allStepsCompleted() ? (
                        ...
                        <React.Fragment>
                            <div > <b> Paso actual</b>: {steps[activeStep]}</div>
