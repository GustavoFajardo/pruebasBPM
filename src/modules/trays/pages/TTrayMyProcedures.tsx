...
export const TTrayMyProcedures: React.FC<ITrayMyProcedures> = () => {
    ...
    const llamarUsuario = () => {
        if (getSesion()) {
            return `${getSesion().Name1} ${getSesion().Surname1} - ${getSesion().Nit} `;
        } else {
            return '';
        }
    }
    ...
    const openForm = (statusForm: boolean) => {
        ...
        if (statusForm === false) {
            getProcedureActionByAccount(beanProcedureImp.IDProcedureImp, parseInt(getSesion().IDAccount));
        }
    }
    ...
    const listarMisProcedimientos = async () => {
        setShowSpinner(true);
        await _suscripcionService.getProcedureImpForInput(parseInt(getSesion().IDAccount)).then((resp: any) => {
    ...
    const seleccionar = (bean: any) => {
        ...
        getProcedureActionByAccount(bean.IDProcedureImp, parseInt(getSesion().IDAccount));
        handleComplete();
    }
