...
export const TTrayForRejected: React.FC<ITrayForRejected> = () => {
    ...
    const listarProcedimientosDevueltos = async () => {
        setShowSpinner(true);
        await _suscripcionService.getProcedureImpRejectedForInput(parseInt(getSesion().IDAccount)).then((resp: any) => {
    ...
    const getProcedureActionRejected = async (idImp: number) => {
        await _suscripcionService.getProcedureActionRejected(parseInt(getSesion().IDAccount), idImp).then((resp: any) => {
            if (resp.data.DataBeanProperties.ObjectValue) {
        ...
        switch (param) {
            ...
            case 1: return <div>
                <div className="w-100 d-flex flex-wrap justify-content-lg-start justify-content-sm-center">
                    ...
                    {showModalObs && (
                        <Modal show={showModalObs} backdrop="static" centered keyboard={false}>
                            <Modal.Header>
                                Observaciones
                                <BsXSquare onClick={() => setShowModalObs(false)} />
                            </Modal.Header>
                            <Modal.Body>
