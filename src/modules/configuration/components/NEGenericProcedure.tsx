...
import { IconButton, Button, InputAdornment, MenuItem, TextField, FormGroup, FormControlLabel, Switch, Typography, ThemeProvider, Checkbox, CircularProgress } from "@mui/material";
import { useEffect, useState } from "react";
import { Modal, Row, Col } from "react-bootstrap";
import { BsXSquare, BsSearch, BsJustifyRight } from "react-icons/bs";
...
import { TDocumentCharacterization } from "../components/TDocumentCharacterization";
...
const NEGenericProcedure: React.FC<INEGenericProcedure> = (
  ...
  const [documentType, setDocumentType] = useState<number | null>(props.dataTitleNE === "Editar" ? props.dataObjNe.DocumentType : null);
  const [applyForAllChar, setApplyForAllChar] = useState((props.dataTitleNE === "Editar") ? props.dataObjNe.ApplyForAllChar : false);
  const [isOptional, setIsOptional] = useState((props.dataTitleNE === "Editar") ? props.dataObjNe.IsOptional : false);

  const [formType, setFormType] = useState<number | null>(props.dataTitleNE === "Editar" ? props.dataObjNe.IDFormClass : null);
  const [formValue, setFormValue] = useState<number | null>(props.dataTitleNE === "Editar" ? props.dataObjNe.IDForm : null);
  const [serviceType, setServiceType] = useState<number | null>(props.dataTitleNE === "Editar" ? props.dataObjNe.IDJsonServiceClass : null);
  const [serviceValue, setServiceValue] = useState<number | null>(props.dataTitleNE === "Editar" ? props.dataObjNe.IDJsonService : null);
  ...
  const onSubmit = (data: any, e: any) => {
    ...
    aux.IsOptional = applyForAllChar;
    console.log(aux);
  ...
  const onChangeComponent = (type: number, data: any) => {
    if (type === 2) { getFormCatalog(parseInt(data)) }
    else if (type === 3) { getJsonServiceCatalog(parseInt(data)) }
  }
    ...
    <>
      ...
      >
        <Modal.Header>
          ...
          <BsXSquare onClick={closeModalNE} />
        </Modal.Header>
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Row>
              ...
              <>
                ...
                {/* <Col sm={6} className="mt-3">
                    ...
                    <span className="text-danger">
                      {errors.entity
                        ? errors.entity.AreaName?.type === "required" &&
                        "El campo 'Area funcional' es requerido."
                        : ""}
                    </span>
                  </Col> */}
                <Col sm={4} className="mt-3">
                  <TextField
                    ...
                    onChange={(e: any) => {
                      ...
                      // if (e.target.value === 3) { getFormClassCatalog(); }
                      if (e.target.value === 6) { getFormClassCatalog(); }
                      else if (e.target.value === 4) { getJsonServiceClassCatalog() }
                    }}
                  >
                  ...
                  <>
                    <Col sm={4} className="mt-3">
                      <TextField
                        ...
                        onChange={(e: any) => { onChangeComponent(2, e.target.value); setFormType(e.target.value) }}
                      >
                    ...
                    <Col sm={4} className="mt-3">
                      {showSpinner
                        ...
                        <TextField
                          ...
                          label="Formulario *"
                          id="state"
                          {...register("entity.IDForm", { required: true })}
                          onChange={(e: any) => setFormValue(e.target.value)}
                        >
                          {list.map((item: any) => (
                            <MenuItem value={item.IDForm}>
                              {item.Name}
                            </MenuItem>
                          ))}
                        </TextField>
                  ...
                  <>
                    <Col sm={4} className="mt-3">
                      <TextField
                        ...
                        id="state"
                        {...register("entity.IDJsonServiceClass", { required: true })}
                        onChange={(e: any) => { onChangeComponent(3, e.target.value); setServiceType(e.target.value) }}
                      >
                      ...
                      <span className="text-danger">
                        {errors.entity
                          ? errors.entity.IDJsonServiceClass?.type === "required" &&
                    ...
                    <Col sm={4} className="mt-3">
                      {(serviceType !== null && listService.length > 0) &&
                        <TextField
                          ...
                          label="Servicio: *"
                          id="state"
                          {...register("entity.IDJsonService", { required: true })}
                ...
                {(documentType === 3) &&
                  <>
                    <Col sm={4} className="mt-3">
                      ...
                      <span className="text-danger">
                        {errors.entity
                          ? errors.entity.FormURLComponent?.type === "required" &&
                ...
                <Col sm={6} className=" sm-12  d-flex justify-content-center mt-3">
                  <FormGroup>
                    <FormControlLabel control={
                      <ThemeProvider theme={inputsTheme}>
                        <Checkbox color="secondary" defaultChecked={applyForAllChar} onChange={(e) => handleChange(e)} />
                      </ThemeProvider>
                ...
                <Col sm={6} className=" sm-12  d-flex justify-content-center mt-3">
                  <FormGroup>
                    <FormControlLabel control={
                      <ThemeProvider theme={inputsTheme}>
                        <Checkbox color="secondary" defaultChecked={isOptional} onChange={(e) => handleChange2(e)} />
                      </ThemeProvider>
                    } label="¿Es Opcional?" />
                  </FormGroup>
                </Col>
          ...
          <Modal.Footer>
            <div className="modal-element">
              <Button variant="contained" color="error" onClick={closeModalNE}>CANCELAR</Button>
            </div>
            <div className="modal-element">
